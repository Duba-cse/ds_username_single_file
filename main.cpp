...
// Highly optimized parameters to achieve ~50 false positives
...
void interactive_mode_all(
    ...
    while(true){
        ...
        vector<long long> search_latency(5);
        vector<bool> found_status(5,false);

        auto start=high_resolution_clock::now(); found_status[0]=hm.search(input); auto end=high_resolution_clock::now();
        search_latency[0]=duration_cast<nanoseconds>(end-start).count();

        start=high_resolution_clock::now(); found_status[1]=bloom.search(input); end=high_resolution_clock::now();
        search_latency[1]=duration_cast<nanoseconds>(end-start).count();

        start=high_resolution_clock::now(); found_status[2]=cb.search(input); end=high_resolution_clock::now();
        search_latency[2]=duration_cast<nanoseconds>(end-start).count();

        start=high_resolution_clock::now(); found_status[3]=trie.search(input); end=high_resolution_clock::now();
        search_latency[3]=duration_cast<nanoseconds>(end-start).count();

        start=high_resolution_clock::now(); found_status[4]=cf.search(input); end=high_resolution_clock::now();
        search_latency[4]=duration_cast<nanoseconds>(end-start).count();
        ...
        if(can_insert){
            ...
            cin>>choice;
            if(choice=='y'){
                vector<long long> insert_latency(5);
                if(idx==-1){ all_usernames.push_back(input); idx=all_usernames.size()-1;
                    for(int i=0;i<5;i++) structure_status[i].push_back(false); }

                start=high_resolution_clock::now(); if(!structure_status[0][idx]) hm.insert(input); end=high_resolution_clock::now();
                insert_latency[0]=duration_cast<nanoseconds>(end-start).count();
                if(!structure_status[0][idx]) structure_status[0][idx]=true;

                start=high_resolution_clock::now(); if(!structure_status[1][idx]) bloom.insert(input); end=high_resolution_clock::now();
                insert_latency[1]=duration_cast<nanoseconds>(end-start).count();
                if(!structure_status[1][idx]) structure_status[1][idx]=true;

                start=high_resolution_clock::now(); if(!structure_status[2][idx]) cb.insert(input); end=high_resolution_clock::now();
                insert_latency[2]=duration_cast<nanoseconds>(end-start).count();
                if(!structure_status[2][idx]) structure_status[2][idx]=true;

                start=high_resolution_clock::now(); if(!structure_status[3][idx]) trie.insert(input); end=high_resolution_clock::now();
                insert_latency[3]=duration_cast<nanoseconds>(end-start).count();
                if(!structure_status[3][idx]) structure_status[3][idx]=true;

                start=high_resolution_clock::now(); if(!structure_status[4][idx]) cf.insert(input); end=high_resolution_clock::now();
                insert_latency[4]=duration_cast<nanoseconds>(end-start).count();
                if(!structure_status[4][idx]) structure_status[4][idx]=true;
            ...
            cin>>choice;
            if(choice=='y'){
                vector<long long> delete_latency(3);
                start=high_resolution_clock::now(); hm.remove(input); end=high_resolution_clock::now();
                delete_latency[0]=duration_cast<nanoseconds>(end-start).count();
                structure_status[0][idx]=false;

                start=high_resolution_clock::now(); cb.remove(input); end=high_resolution_clock::now();
                delete_latency[1]=duration_cast<nanoseconds>(end-start).count();
                structure_status[2][idx]=false;

                start=high_resolution_clock::now(); cf.remove(input); end=high_resolution_clock::now();
                delete_latency[2]=duration_cast<nanoseconds>(end-start).count();
...
int main(){
    ...
    cout << "  • Cuckoo Filter:         150K buckets, 8-bit fingerprints, 4 entries/bucket → Expected FP Count: ~50\n";
    ...
    HashMapChecker hm_temp; 
    auto start_time = high_resolution_clock::now();
    insert_bulk(hm_temp, to_insert);
    auto end_time = high_resolution_clock::now();
    insert_latencies[0] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / to_insert.size();

    BloomFilterChecker bloom_temp;
    start_time = high_resolution_clock::now();
    insert_bulk(bloom_temp, to_insert);
    end_time = high_resolution_clock::now();
    insert_latencies[1] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / to_insert.size();

    CountingBloomChecker cb_temp;
    start_time = high_resolution_clock::now();
    insert_bulk(cb_temp, to_insert);
    end_time = high_resolution_clock::now();
    insert_latencies[2] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / to_insert.size();

    TrieChecker trie_temp;
    start_time = high_resolution_clock::now();
    insert_bulk(trie_temp, to_insert);
    end_time = high_resolution_clock::now();
    insert_latencies[3] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / to_insert.size();

    CuckooFilterChecker cf_temp;
    start_time = high_resolution_clock::now();
    insert_bulk(cf_temp, to_insert);
    end_time = high_resolution_clock::now();
    insert_latencies[4] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / to_insert.size();

    cout << "Measuring search latency..." << endl;
    start_time = high_resolution_clock::now();
    check_bulk(hm, queries);
    end_time = high_resolution_clock::now();
    search_latencies[0] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / queries.size();

    start_time = high_resolution_clock::now();
    check_bulk(bloom, queries);
    end_time = high_resolution_clock::now();
    search_latencies[1] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / queries.size();

    start_time = high_resolution_clock::now();
    check_bulk(cb, queries);
    end_time = high_resolution_clock::now();
    search_latencies[2] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / queries.size();

    start_time = high_resolution_clock::now();
    check_bulk(trie, queries);
    end_time = high_resolution_clock::now();
    search_latencies[3] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / queries.size();

    start_time = high_resolution_clock::now();
    check_bulk(cf, queries);
    end_time = high_resolution_clock::now();
    search_latencies[4] = static_cast<double>(duration_cast<nanoseconds>(end_time - start_time).count()) / queries.size();
